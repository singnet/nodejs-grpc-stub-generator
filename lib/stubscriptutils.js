...
export const extractStubs = async (protoFilePath, output_path) => {
  const protoc = validateFileLocation(path.resolve("./node_modules/grpc-tools/bin/protoc"));
  const nodeplugin = validateFileLocation(path.resolve("./node_modules/.bin/grpc_tools_node_protoc_plugin"));
  const jsplugin = validateFileLocation(path.resolve("./node_modules/.bin/protoc-gen-ts"));
